...
module Yast
  class SCTHostClass < Module
    ...
    def Read
      ...
      # read hosts
      f = File.open(@hosts_file, "r") if File.exists?(@hosts_file)
      if f
        f.each do |line|
          case line
            when /^[0-9abcdefABCDEF\:]/
              line = line.gsub(/\s+/m, ' ').split(" ")
              @hosts[line[0]] = [line.slice(1, line.length - 1).join(" ")]
            end
          end
        f.close()
      end
    ...
    publish function: :clear, type: "void ()"
    publish :variable => :hosts_file, :type => "string"
  end

  SCTHost = SCTHostClass.new
  SCTHost.main
end
