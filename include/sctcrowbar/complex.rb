...
#
module Yast
  module SctcrowbarComplexInclude
    def initialize_sctcrowbar_complex(include_target)
      Yast.import "UI"
      ...
      Yast.include include_target, "sctcrowbar/helps.rb"
    ...
    def ReallyAbort
      !SCTCrowbar.Modified || Popup.ReallyAbort(true)
    end
    ...
    def InitMode(id)
      ...
      UI.ChangeWidget(Id(id), :Items, items)
      UI.ChangeWidget(Id(id), :Enabled, !SCTCrowbar.installed)
      nil
    end
    ...
    def HandleRangesButton(key, event)
      ...
      ranges.each do |name, range|
        ...
        UI.ChangeWidget(Id(name + "_start"), :Enabled, !SCTCrowbar.installed)
        UI.ChangeWidget(Id(name + "_end"), :Enabled, !SCTCrowbar.installed)
      end

      UI.ChangeWidget(Id(:ok), :Enabled, !SCTCrowbar.installed)
    ...
    def OverviewDialog
      @networks         = SCTCrowbar.networks
      @mode             = SCTCrowbar.mode
      @teaming          = SCTCrowbar.teaming
      @users            = SCTCrowbar.users
      @repos            = SCTCrowbar.repos
      @conduit_map      = SCTCrowbar.conduit_map
      ...
      # not saving
      return :back if SCTCrowbar.installed
      ...
      if ret == :next
        ...
        SCTCrowbar.networks = @networks
        SCTCrowbar.conduit_map = @conduit_map # was adapted by adapt_conduit_map
        SCTCrowbar.users = @users
        SCTCrowbar.teaming = @teaming
        SCTCrowbar.mode = @mode
        SCTCrowbar.repos = @repos
      end
      ret
    end
  end
