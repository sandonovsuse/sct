...
module Yast
  module SctHostInclude
    def initialize_sct_host(include_target)
      Yast.import "UI"
        ...
        # delete host
          ...
          if SCTHost.GetModified
            ...
            Builtins.foreach(table_items) do |row|
              ...
              SCTHost.add_name(key, value)
            end
            # deleted entries need to be set to [],
            # so that ini-agent does not keep them in
            # config file (#455862)
            Builtins.foreach(deleted_items) { |d| SCTHost.set_names(d, []) }
          end
          break
        else
